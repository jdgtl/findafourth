...
const AppLayout = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Top Navigation */}
      <header className="sticky top-0 z-50" style={{ background: 'rgba(10,15,26,0.95)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(52,211,153,0.1)' }}>
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-16">
            <Link to="/home" className="flex items-center gap-2">
              ...
              <span className="text-lg font-serif text-warm tracking-tight">Find4th</span>
            </Link>
            ...
            <nav className="hidden md:flex items-center space-x-1">
              {navItems.map(({ path, icon: Icon, label }) => (
                <Link key={path} to={path}>
                  <Button
                    variant="ghost"
                    className={`flex items-center gap-2 hover:text-emerald-400 hover:bg-transparent ${
                      location.pathname === path ? 'text-emerald-400' : 'text-warm-muted'
                    }`}
                  >
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="flex items-center gap-2 text-warm-muted hover:text-emerald-400 hover:bg-transparent">
